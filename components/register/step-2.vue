  ...
  <v-container fluid class="component-main-container">
    <v-row justify="center" align="center">
      ...
      <v-col cols="12" class="plans-col">
        <v-row :class="['plans-row', mobile ? '' : 'mx-10']" :align="mobile ? null : 'start'" dir="ltr">
          <v-col
            v-for="(plan, index) in plans"
            :key="index"
            :cols="mobile ? 12 : false"
            :class="[mobile ? '' : 'ma-4', 'd-flex', 'justify-center', 'align-center']"
            v-intersect="{
              handler: (entries) => {
          ...
          >
            <v-card
              ...
              rounded="xl"
            >
              <v-card-title class="text-center text-h4 mt-4">{{ plan.name }}</v-card-title>
              ...
              <v-card-subtitle :class="[annual ? 'opacity-0' : '', 'opacity-transition-fade', 'text-center']" >
...
export default {
  ...
  methods: {
    ...
    async planButtonHandler(plan) {
      const res = await $fetch('/api/plans/getCheckout', {
        method: 'POST',
        body: {
          token: await this.user.getIdToken(),
          priceID: plan.prices[this.currency][this.annual ? 'monthly' : 'annual'].id,
        }
      });
      window.location = res;
    },
